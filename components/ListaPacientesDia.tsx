...
export default function ListaPacientesDia({ turnos, fecha }: ListaPacientesDiaProps) {
  ...
  return (
    <>
      ...
      <div className="print-lista-pacientes hidden print:block">
        <div className="p-8">
          {/* Encabezado */}
          ...
          {turnosOrdenados.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              No hay pacientes con turno programado para este día
            </div>
          ) : (
            <table className="w-full border-collapse">
              ...
              <tbody>
                {turnosOrdenados.map((turno, index) => (
                  <tr key={turno.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
          ...
          {turnosOrdenados.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-gray-100">
                    <th className="border border-gray-300 px-3 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-bold text-gray-900">
                      Nro. Ficha
                    </th>
                    <th className="border border-gray-300 px-3 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-bold text-gray-900">
                      Apellido
                ...
                <tbody>
                  {turnosOrdenados.map((turno, index) => (
                    ...
                    >
                      <td className="border border-gray-300 px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm text-gray-900 font-medium">
                        {turno.pacientes.numero_ficha || '-'}
                      </td>
                      <td className="border border-gray-300 px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm text-gray-900 font-semibold">
                        {turno.pacientes.apellido}
                      </td>
                      <td className="border border-gray-300 px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm text-gray-900 font-semibold">
                        {turno.pacientes.nombre}
